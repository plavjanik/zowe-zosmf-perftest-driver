...
async function initializeImperative() {
    const mainZoweDir = path_1.join(require.resolve('@zowe/cli'), '..', '..', '..', '..');
    process.mainModule.filename = require.resolve('@zowe/cli');
    process.mainModule.paths.unshift(mainZoweDir);
    await imperative_1.Imperative.init({ configurationModule: require.resolve('@zowe/cli/lib/imperative.js') });
}
...
class Zztop extends command_1.Command {
    async userActivity(userNumber, testDefinition, zosmfProfilesByName) {
        const scriptDelay = parse_duration_1.default(testDefinition.scriptDelay) || 1000;
        const commandDelay = parse_duration_1.default(testDefinition.commandDelay) || 1000;
        ...
        if (!exists) {
            const response = await cli_1.Create.dataSet(session, 2 /* DATA_SET_CLASSIC */, testDsn);
            if (!response.success) {
        ...
        while (new Date().getTime() - startTime <= duration) {
            ...
            const uploadResponse = await cli_1.Upload.fileToDataset(session, tmpCobolPath, testDsn + '(TEST1)'); // eslint-disable-line no-await-in-loop
            perf_timing_1.PerfTiming.api.mark('AfterDatasetUpload');
            perf_timing_1.PerfTiming.api.measure('DatasetUpload', 'BeforeDatasetUpload', 'AfterDatasetUpload');
            ...
            await cli_1.sleep(Math.max(commandDelay - (new Date().getTime() - commandStartTime), 0)); // eslint-disable-line no-await-in-loop
            await cli_1.sleep(Math.max(scriptDelay - (new Date().getTime() - scriptStartTime), 0)); // eslint-disable-line no-await-in-loop
        }
    ...
    async run() {
        ...
        const profiles = await new imperative_1.CliProfileManager({
            profileRootDirectory: path_1.join(imperative_1.ImperativeConfig.instance.cliHome, 'profiles'),
            type: 'zosmf',
        }).loadAll();
        ...
        for (const measurement of perf_timing_1.PerfTiming.api.getMetrics().measurements) {
...
Zztop.args = [{ name: 'file', required: true, description: 'Test definition file' }];
